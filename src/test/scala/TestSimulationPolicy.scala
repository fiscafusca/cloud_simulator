...
class TestSimulationPolicy extends FlatSpec {
  ...
  it should "return a container of " + cloudletsNum + " new cloudlets" in {
    val cloudlets: List[Cloudlet] = SimulationPolicy.createCloudlet(conf.getInt("main.num_user"), conf.getInt("main.idShift"))
  ...
  it should "return a container of " + vmNum + " new VMs" in {
    val vms: List[Vm] = SimulationPolicy.createVM(conf.getInt("main.num_user"), conf.getInt("main.idShift"))
  ...
  it should "return a new object of type Datacenter" in {
    val datacenter = SimulationPolicy.createDatacenter("test_datacenter")
  ...
  it should "return a new object of type MasterNode with name set to " + masterNodeName in {
    val masterNode = SimulationPolicy.createMasterNode(masterNodeId)
